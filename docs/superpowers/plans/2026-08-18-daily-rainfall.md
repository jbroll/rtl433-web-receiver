...
**Architecture:** A new `device_hooks` module holds the rain baseline logic and a model→hook registry. Hooks take the `JsonDocument` directly and may add or modify any field. `signal_store` gains an optional `RecordHook` callback it calls after parsing and stamping, before storing. A `tz_store` module persists a signed GMT offset in NVS; the dashboard POSTs the offset to `/$tz` when the location is set.
...
- Modules without other Arduino headers host-tested by `bash test/host/run.sh` with g++ -std=c++17 -Wall -Wextra -Werror. `device_hooks` includes ArduinoJson and compiles against the headers PlatformIO fetches into `receiver/.pio/libdeps`.
...
Hooks take `JsonDocument&` and read and write fields on it directly, so a future sensor type needs only a new hook function, not a new struct. `device_hooks` includes ArduinoJson (already a firmware dependency) and host-tests against the libdeps headers. Only `rain_mm` is handled: the Acurite-5n1's rtl_433 decoder reports rainfall in mm only, so there is no `rain_in` fallback.
...
**Interfaces:**
- Consumes: nothing (own TZ offset and time override)
- Produces:
  - `typedef void (*Hook)(const char* key, JsonDocument& doc)` — the hook reads `doc["rain_mm"]` and writes `doc["rain_today_mm"]`
  ...
  - `void dispatch(const char* key, JsonDocument& doc)` — reads `doc["model"]`, calls the registered hook
...
```sh
#!/bin/sh
# topic.cpp and radio_health.cpp are Arduino-free firmware modules; their rules
# are checked here rather than by compilation alone. device_hooks.cpp includes
# ArduinoJson, fetched by PlatformIO into libdeps.
set -e
...
**Interfaces:**
- Consumes: `device_hooks::dispatch` (from Task 1)
- Produces:
  - `typedef void (*RecordHook)(const char* key, JsonDocument& doc)`
  - `void setRecordHook(RecordHook hook)`
  - `record()` calls the hook (if set) with its `JsonDocument` after stamping time/rssi/count, before the size check
...
The hook receives the `JsonDocument` and may add or modify fields on it; the size check then measures the augmented doc.
