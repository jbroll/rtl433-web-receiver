...
# Usage: tools/fetch_coredump.sh [serial-port] [pio-environment]
set -e
port=${1:-/dev/ttyACM0}
pioenv=${2:-rfm69-433}
here=$(cd "$(dirname "$0")" && pwd)
build="$here/../.pio/build/$pioenv"
...
elf="$here/elf/$build_id-$pioenv.elf"
# ELFs saved before the environment suffix existed are all rfm69-433 builds.
[ -f "$elf" ] || [ "$pioenv" != rfm69-433 ] || elf="$here/elf/$build_id.elf"
...
# The rfm69-433-fakesignals environment builds to its own directory and
...
if strings "$elf" 2>/dev/null | grep -q '^fakeSignal'; then
    ...
    echo "it will not symbolicate a real coredump correctly; rebuild $pioenv and retry" >&2
    exit 1
fi
