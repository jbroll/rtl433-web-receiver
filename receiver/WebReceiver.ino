...
// The receiver's own I2C sensors, recorded as one device so the page renders
// them with everything it already does for a sensor. rssi is the WiFi link,
// which is what the card's corner reading means for this one. Each field has
// one source sensor, so a failed read leaves that sensor's fields out.
static void recordLocalI2C() {
  char   buf[JSON_MSG_BUFFER];
  size_t at  = appendf(buf, sizeof(buf), 0, "{\"model\":\"local-i2c\",\"id\":\"local-i2c\"");
  bool   any = false;
  if (bmp280_ok) {
    ...
    if (isnan(t) || isnan(p)) {
      ...
      at  = appendf(buf, sizeof(buf), at, ",\"temperature_C\":%.2f,\"pressure_hPa\":%.2f", t, p);
      any = true;
    }
  }
  if (aht20_ok) {
    float t, h;
    if (aht20::read(t, h)) {
      at  = appendf(buf, sizeof(buf), at, ",\"humidity\":%.1f,\"aht20_temperature_C\":%.2f", h, t);
      any = true;
    } else {
  ...
  Log.notice(F("local-i2c: %s" CR), buf);
...
void setup() {
  ...
  if (aht20_ok) {
    Log.notice(F("AHT20 initialized at %X" CR), aht20::ADDR);
  } else {
...
void loop() {
  ...
  static unsigned long lastLocalI2C = 0;
  if (millis() - lastLocalI2C >= 30000) {
    lastLocalI2C = millis();
    recordLocalI2C();
  }
