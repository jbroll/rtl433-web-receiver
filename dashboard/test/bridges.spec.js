...
test("removing a bridge that was never there answers 404, and the row stays with a toast", async ({ page }) => {
  await openBridges(page, [{ url: "mqtts://weather.rkroll.com:8883", connected: true }]);
  await page.route("**/$mqtt/remove", (route) => route.fulfill({ status: 404 }));
